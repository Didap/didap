...
<template>
  <header
    ref="headerEl"
    class="fixed top-4 left-4 right-4 md:left-8 md:right-8 z-[100] flex justify-between items-center px-6 md:px-8 py-3.5 rounded-2xl transition-all duration-500 will-change-transform"
    :class="
      isScrolled
  ...
  >
    ...
    <nav class="hidden md:flex items-center gap-10">
      <button
        ...
        class="text-[0.7rem] font-bold uppercase tracking-[0.2em] transition-colors duration-300 relative group"
      ...
      >
        ...
        <span
          v-if="isActive(link)"
          class="absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-orange-500"
        ></span>
      </button>
    </nav>
    ...
    <a
      ...
      class="hidden md:inline-flex px-6 py-2.5 bg-slate-900 text-white text-[0.65rem] font-bold uppercase tracking-[0.15em] rounded-full hover:bg-orange-500 transition-all duration-300 shadow-lg hover:shadow-orange-500/20 active:scale-95"
    >
      Lavora con noi
    </a>
    ...
    >
      <span
        class="w-5 h-0.5 bg-slate-900 rounded-full transition-all duration-300"
