...
const glowSweepStyle = ref('')
let autoTiltTimeline: gsap.core.Timeline | null = null
let glowSweepTimeline: gsap.core.Tween | null = null
let isUserInteracting = false

const autoTiltData = { mx: 0.5, my: 0.5 }

const applyTilt = (mx: number, my: number) => {
  const rotateX = (my - 0.5) * -15
  const rotateY = (mx - 0.5) * 15
  ...
  shineStyle.value = `background: radial-gradient(circle at ${mx * 100}% ${my * 100}%, rgba(255,255,255,${shineOpacity}) 0%, transparent 60%); opacity: 1;`

  if (props.rarity !== 'common') {
    const holoPos = mx * 50 + my * 50
...
const startAutoTilt = () => {
  ...
  autoTiltTimeline = gsap.timeline({ repeat: -1, yoyo: true })
    .to(autoTiltData, {
      mx: 0.85,
      my: 0.15,
      duration: 3,
      ease: 'sine.inOut',
      onUpdate: () => applyTilt(autoTiltData.mx, autoTiltData.my)
    })
    .to(autoTiltData, {
      mx: 0.15,
      my: 0.85,
...
const startGlowSweep = () => {
  const sweepData = { pos: -100 }
  const delay = props.rarity === 'ultra' ? 4 : props.rarity === 'rare' ? 5 : 6

  glowSweepTimeline = gsap.to(sweepData, {
    pos: 200,
    duration: 1.2,
    ease: 'power2.inOut',
    repeat: -1,
    repeatDelay: delay,
    onUpdate: () => {
      const opacity = props.rarity === 'ultra' ? 0.5 : props.rarity === 'rare' ? 0.35 : 0.2
      glowSweepStyle.value = `background: linear-gradient(105deg, transparent ${sweepData.pos - 30}%, rgba(255,255,255,${opacity}) ${sweepData.pos}%, transparent ${sweepData.pos + 30}%); opacity: 1;`
    },
    onRepeat: () => {
      sweepData.pos = -100
    },
...
const stopAutoTilt = () => {
  if (autoTiltTimeline) {
    autoTiltTimeline.kill()
    autoTiltTimeline = null
  }
  if (glowSweepTimeline) {
    glowSweepTimeline.kill()
    glowSweepTimeline = null
  }
}
...
const resetTilt = () => {
  ...
  if (isTouchDevice.value) {
    if (autoTiltTimeline) autoTiltTimeline.resume()
  } else {
    // Smooth reset
    gsap.to(autoTiltData, {
      mx: 0.5,
      my: 0.5,
      duration: 0.5,
      ease: 'power2.out',
...
<template>
  <div ref="cardRef"
    class="relative w-full max-w-[340px] h-[560px] rounded-[1.8rem] transition-all duration-300 ease-out cursor-pointer select-none group overflow-hidden main-card-container"
    :class="cardBaseClasses" :style="{ transform: tiltStyle }" @mousemove="handleMouseMove" @mouseleave="resetTilt">
    ...
    <template v-if="rarity === 'ultra'">
      <div class="absolute inset-0 z-0 transform-gpu">
        ...
        <div class="absolute inset-0 bg-linear-to-b from-black/10 via-transparent to-black/80"></div>
      </div>
    ...
    <template v-else>
      <div class="absolute inset-0 z-0 p-4 flex flex-col"
        :class="rarity === 'common' ? 'bg-slate-200' : 'bg-slate-900'">
        <!-- Header area for Common/Rare -->
        <div class="flex justify-between items-center mb-4 px-1">
          <h2 class="text-xl font-black uppercase tracking-tight font-display"
            :class="rarity === 'common' ? 'text-slate-800' : 'text-white'">
            {{ name }}
          </h2>
          <div class="text-2xl font-black italic select-none font-heading" :class="rarity === 'common'
        ...
        <div class="mt-4 flex flex-wrap gap-1.5 px-1">
          <span v-for="tech in techStack.slice(0, 3)" :key="tech"
            class="px-2 py-0.5 rounded text-[0.55rem] font-bold uppercase tracking-widest border" :class="rarity === 'common'
        ...
        <div class="mt-6 flex-1 px-1">
          <p class="text-[0.75rem] italic font-medium leading-relaxed"
          ...
          <div class="mt-6 space-y-4">
            <div v-for="ability in abilities" :key="ability.name" class="flex flex-col">
              <h3 class="text-[0.65rem] font-black uppercase tracking-widest"
    ...
    <div v-if="rarity === 'ultra'" class="relative h-full w-full flex flex-col p-6 z-30 transform-gpu">
      <div class="flex justify-between items-start mb-auto">
        <div class="flex flex-col drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">
          <span class="text-[0.65rem] font-black text-white/70 uppercase tracking-[0.25em] mb-1 font-heading">
            {{ level || 'VMAX VERSION' }}
          </span>
          <h2
            class="text-3xl font-black text-white uppercase tracking-tighter leading-none skew-x-[-10deg] shadow-text font-display">
            {{ name }}
          </h2>
        </div>
        <div class="flex flex-col items-end drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">
          <div class="text-4xl font-black italic leading-none font-heading" :class="typeGlows[type || 'electric']">
            {{ year }}<span class="text-xl opacity-60">.{{ month }}</span>
          </div>
          <span class="text-[0.6rem] font-bold text-white/60 uppercase tracking-widest mt-1 font-heading">Full Art Core</span>
        </div>
      </div>
    ...
    <!-- GLOBAL GLARE/SHINE LAYER (always visible for auto-animation) -->
    <div
      class="absolute inset-0 pointer-events-none z-50 mix-blend-overlay transition-opacity duration-300 transform-gpu"
      :style="shineStyle"></div>
    <!-- GLOW SWEEP LAYER -->
    <div class="absolute inset-0 pointer-events-none z-50 rounded-[1.8rem] overflow-hidden transform-gpu"
      :style="glowSweepStyle"></div>
    <!-- ULTRA/RARE Shine Overlay (always visible but faint) -->
    <div v-if="rarity !== 'common'"
